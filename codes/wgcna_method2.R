...
expressiondata <- read.delim("/Users/rishav_raj/Desktop/bioinformatics/geo/GSE_raw.tsv", sep = "\t", header = TRUE)

gse <- getGEO("GSE153659")
length(gse)
gse <- gse[[1]]
gse
phenod <- pData(gse)
...
#Column 1 -  gene names
colnames(expression) = expressiondata$GeneID
...
abline(h = 1500000, col = "red") #This value of 31000 was chosen based on my data, you need to check the best value to your data
...
traitData = read.csv("/Users/rishav_raj/Desktop/bioinformatics/geo/traits_GSE.txt", sep = ",",header=TRUE)
traitData <- traitData[,names(traitData) %in% c("Sample.Name","SAMPLE_TYPE")]
dim(traitData)
names(traitData)
Samples = rownames(expression)

traitRows = match(Samples, traitData$Sample.Name)
datTraits = as.data.frame(traitData[traitRows, -1])
names(datTraits) <- c("SAMPLE_TYPE")
rownames(datTraits) = traitData[traitRows, 1]
datTraits["TYPE"] <- ifelse(datTraits$SAMPLE_TYPE=="papillary thyroid carcinoma",1,0)
collectGarbage()
...
traitColors = numbers2colors(datTraits$TYPE, signed = FALSE)
